...
int process_handler(int sock, indiv &user, char *ptr, int myid);//, vector<int> &fd, vector<string> &hstr, vector<string> &pstr);
...
int process_handler(int sock, indiv &user, char *ptr, int myid)//, vector<int> &fd, vector<string> &hstr, vector<string> &pstr)
{
	...
	while(1)
	{
		...
		if(cmd_args[cur][0] == "exit")return 0;
		...
		if(cmd_args[cur][0] == "name")
		{
			...
			for(int i = 0; i < 30; i++)
			{
				if(readmmry(ptr + NAME_OFFSET + i * NAME) == cmd_args[cur][1])
				{
					string str = string() + "*** User '" + cmd_args[cur][1] + "' already exists. ***\n";
					write(sock, str.c_str(), str.size());
					return 1;
				}
			}
			string str2;
			str2 = str2 + "*** User from CGILAB/511 is named '" + cmd_args[cur][1] + "'. ***\n";
			rstmmry(ptr + NAME_OFFSET + NAME * myid, NAME);
			writemmry(ptr + NAME_OFFSET + NAME * myid, cmd_args[cur][1].c_str());
		...
		*/
		...
		if(!ptr[0]) 
		{
			cout << "[WRN] Exec error: " << argc.data()[0] << endl;
